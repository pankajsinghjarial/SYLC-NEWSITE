...
<link href="<?php echo DEFAULT_ADMIN_URL?>/custinfo/css/cansole.css" rel="stylesheet" type="text/css" media="all">
 ...
 <div class="account_box no_bdr" id="inline_content_edit<?php echo $j;?>">
    ...
    <form method="post" action="<?php echo DEFAULT_ADMIN_URL?>/customer_info.php" onsubmit="return feescheck(<?php echo $j;?>)">
    <ul>
    ...
    <option value="">Please Select</option>
     <?php 
      ...
      while($feefetch = mysql_fetch_object($feeinfo)) {?>
         <option value="<?php echo str_replace(",","",$feefetch->fees).'~'.$feefetch->title?>" <?php if($feefetch->title == $editfeefetch->title) { ?> selected="selected" <?php }?>><?php echo $feefetch->title;?></option>
      <?php }?>
    </select></li>    
