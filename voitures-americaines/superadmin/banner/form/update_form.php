  ...
  <div id="content_main" class="clearfix">
    <div style="width:78%; float:left;">
      <form action="<?=$_SERVER['REQUEST_URI'];?>" method="post" name="add_form" id="add_form" enctype="multipart/form-data" >
        ...
        <?php if($old_image!='' && file_exists(LIST_ROOT_ADMIN.'/images/banner/'.$old_image)) {
		$imageInfo=getimagesize(DEFAULT_ADMIN_URL.'/images/banner/'.$old_image);
		$imgWidth=$imageInfo[0];
		if($imgWidth>900)
			$imgWidth='900px';
		?>
        <br /><br />
        	<img src="<?=DEFAULT_ADMIN_URL?>/images/banner/<?=$old_image?>" width="<?php echo $imgWidth; ?>" />        
        <?php }?>
        <br />
        <br />
        </fieldset>
