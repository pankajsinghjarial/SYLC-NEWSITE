...
if(isset($submit) && $submit != "" && $_SERVER['REQUEST_METHOD']=='POST')
	     ...
	     {		
		  ...
		  if($error){
		   ...
		   { 
			...
			if(!empty($_FILES["image"]["name"])) {
					...
					{                            
						...
						/*if(isset($old_image) && $old_image!='' && file_exists(LIST_ROOT_ADMIN."/images/old_model/".$old_image)) {
							unlink(LIST_ROOT_ADMIN."/images/old_model/".$old_image);
						}*/
		   ...
		   $dataArr  =  array('brand_name'=>$brand_name ,'year'=>$year ,'image'=>$image ,'prix'=>$prix ,'contents'=>$contents ,'publish'=>$publish);

		   $update_product=$obj_block->update(TBL_OLD_MODEL,$dataArr,'id='.$model_id);

		   $_SESSION['msg']= 'Old Model Successfully Updated';
         ...
         {		     
			 ...
			 $getBlock= $obj_block->read(TBL_OLD_MODEL,'id='.$model_id);
