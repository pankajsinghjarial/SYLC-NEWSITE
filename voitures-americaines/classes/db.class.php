...
error_reporting(0);
  ...
  class DB
  {
	...
	/* ============ MY new paging function ==========
			CODER RAJEEV ACHRA
			function for general paging on the bases of query
		=================================*/
		public function paging($table)
		{
			extract($_POST);
			extract($_GET);
			$total	=	$this->countOfAll($table);
			 if(!isset($limit))
		  {
		   $limit=8;
		  }
	  ...
	  $limitStart=$limit*$currentPage;
			$paging['pvariable'] = array( 		"limit" 		=> $limit,
                                     			"currentPage" 	=> $currentPage,
                                     			"total" 		=> $total,
                                     			"pagesTotal" 	=> $pagesTotal,
												"limitStart"	=> $limitStart,
                                        );
				return($paging['pvariable']);
			 
		}
//paging function on the bases of query
		public function pagingquery($countquery)
		{
			extract($_POST);
			extract($_GET);
			//echo "==>".$countquery;
