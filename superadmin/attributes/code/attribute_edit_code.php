...
$fetchSetting 			= $obj_setting->read("attribute", 'attribute_id = '.$id);
$getSetting 			= $db->fetchNextObject($fetchSetting);
$id	 					= $getSetting->attribute_id;
$name	 				= $getSetting->frontend_label;
$code				= $getSetting->attribute_code;
$publish 				= $getSetting->publish;
$required 					= $getSetting->is_required;
$searchable 					= $getSetting->searchable;
$type = $getSetting->searchable;
extract($_POST);
...
if($_SERVER['REQUEST_METHOD']=='POST')
    {
		  ...
		  if($error){
		  ...
		  else
		    {
				...
				$dataArr = array('name'=>$name,'slug'=>$slug,'desc'=>$desc, 'publish'=>$publish,'metatitle'=>$metatitle,'metakeyword'=>$metakeyword,'metadescription'=>$metadescription,'can_delete'=>$can_delete);
