...
if(isset($pageid) and $pageid!='' and isset($action) and $action=='delete'){
    ...
    $obj_setting->delete('faq'," id IN('$pageid')");
...
$total_rows = $obj_setting->numberOfRows('faq'," id IN('$totalIds')");
if($total_rows > 0){
	$totalNames     	= array();
	$singlePage 		= $obj_setting->customQuery("SELECT * FROM faq where id IN('$totalIds')");
	...
	$pageName  = 'No FAQ exists with these ids.';
}
unset($obj_setting);
