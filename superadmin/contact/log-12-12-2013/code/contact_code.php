...
if(!isset($_REQUEST['searchcombo'])){
	$total_rows = $obj_setting->numberOfRows('email_log'); //number of rows in pages table
}else{
...
if(!isset($_REQUEST['searchcombo'])){
		...
		{ 
		$all_pages = $obj_setting->customQuery("SELECT * FROM email_log where `email` like '%".addslashes($searchword)."%' order by $field $orderby limit $eu, $limit ");
		}
		else
		{
		$all_pages = $obj_setting->customQuery("SELECT * FROM email_log order by $field $orderby limit $eu, $limit ");
	...
	if($_REQUEST['searchtext']=='Search'){
		...
		$all_pages = $obj_setting->customQuery("SELECT * FROM email_log where `email` like '%".addslashes($_REQUEST['searchtext'])."%' order by $field $orderby limit $eu, $limit "); //number of rows in pages table for perticular search criteria
	}
}
