...
if(!isset($_REQUEST['searchcombo'])){
	$total_rows = $obj_setting->numberOfRows('car_inquiry'); //number of rows in pages table
}else{
...
if(!isset($_REQUEST['searchcombo'])){
		...
		{ 
		$all_pages = $obj_setting->customQuery("SELECT * FROM car_inquiry where type = $type and name like '%".addslashes($searchword)."%' or  `email` like '%".addslashes($searchword)."%' order by $field $orderby limit $eu, $limit ");
		}
		else
		{
		$all_pages = $obj_setting->customQuery("SELECT * FROM car_inquiry order by $field $orderby limit $eu, $limit ");
		}
}else{echo "hii";die;
	if($_REQUEST['searchtext']=='Search'){
		$all_pages = $obj_setting->customQuery("SELECT * FROM car_inquiry order by $field $orderby limit $eu, $limit ");
	}else{
		$all_pages = $obj_setting->customQuery("SELECT * FROM car_inquiry where `name` like '%".addslashes($_REQUEST['searchtext'])."%' OR `desc` like '%".addslashes($_REQUEST['searchtext'])."%' order by $field $orderby limit $eu, $limit "); //number of rows in pages table for perticular search criteria
	}
}

if(isset($_REQUEST['exportMYSQLData'])){
	$mysqlFilename = "general_inquiry_".date("dMY").".csv";
	$CSVraw = $obj_setting->customQuery("SELECT email,phone,message FROM car_inquiry where order by $field $orderby ");
