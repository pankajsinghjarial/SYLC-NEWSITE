...
if(isset($pageid) and $pageid!='' and isset($action) and $action=='delete'){
    ...
    $obj_setting->delete('contact_bid'," id IN('$pageid')");
...
$total_rows = $obj_setting->numberOfRows('contact_bid'," id IN('$totalIds')");
if($total_rows > 0){
	$totalNames     	= array();
	$singlePage 		= $obj_setting->customQuery("SELECT * FROM contact_bid where id IN('$totalIds')");
	...
	$pageName  = 'No Newpage data exists with these ids.';
}
unset($obj_setting);
